...
export const adminApi = {
  getOverview: async () => {
    ...
    const response = await fetch(`${API_BASE_URL}/admin/overview`, { headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' } });
    return handleResponse(response);
  },
  ...
  getVerifications: async () => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/verifications`, { headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' } });
    return handleResponse(response);
  },
  getAuditLogs: async () => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/audit-logs`, { headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' } });
    return handleResponse(response);
  },
  updateInternalAccess: async (empId, data) => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/internal-users/${encodeURIComponent(empId)}/access`, { method: 'PATCH', headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' }, body: JSON.stringify(data) });
    return handleResponse(response);
  },
  updateExternalAccess: async (uid, data) => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/external-users/${encodeURIComponent(uid)}/access`, { method: 'PATCH', headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' }, body: JSON.stringify(data) });
    return handleResponse(response);
  },
  createInternalUser: async (data) => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/internal-users`, { method: 'POST', headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' }, body: JSON.stringify(data) });
    return handleResponse(response);
  },
  updateInternalUser: async (empId, data) => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/admin/internal-users/${encodeURIComponent(empId)}`, { method: 'PUT', headers: { ...getHeaders(), 'X-Admin-Emp-Id': user?.uid || '' }, body: JSON.stringify(data) });
    return handleResponse(response);
  }
};
...
export const paymentsApi = {
  ...
  updateStatus: async (id, status) => {
    const user = JSON.parse(localStorage.getItem('livingora_user') || 'null');
    const response = await fetch(`${API_BASE_URL}/payments/${id}/status`, { method: 'PATCH', headers: { ...getHeaders(), 'X-Staff-Emp-Id': user?.uid || '' }, body: JSON.stringify({ status }) });
    return handleResponse(response);
  }
};
